...
<div class="container-fluid" id="main-content">
    <div class="row">
        <div class="col-lg-10 ms-auto p4 overflow-hidden">
            <br>
            <h3 class="mb-4">Inquries Delete</h3>
    ...
    if ($_SERVER['REQUEST_METHOD']=='POST')
    {
        ...
        {
            //make the query
            $q = "DELETE FROM user_queries WHERE sr_no = $id LIMIT 1";
            $result = @mysqli_query($con,$q);//run the query

            if (mysqli_affected_rows($con)== 1)//if there was a problem
            //display messege
            {
                echo '<script>alert("THE USER INQURIES HAS BEEN DELETED");
        ...
        else
        {
            ...
            echo '<script>alert("THE USER INQURIES HAS NOT BEEN DELETED");
				window.location.href="inquriesList.php";</script>';
        }
    ...
    else
    {
        ...
        $q = "SELECT names FROM user_queries WHERE sr_no = $id";
        $result = @mysqli_query($con,$q);

        if(mysqli_num_rows($result) == 1)
        {
            //get user_queries information
            $row = mysqli_fetch_array($result,MYSQLI_NUM);
            echo"<h3>ARE YOU SURE WANT TO PERMENENTLY DELETE $row[0]? </h3>";
            echo'<form action="inquriesDelete.php" method="POST">
