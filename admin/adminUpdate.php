...
<div class="container-fluid" id="main-content">
	...
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		if(empty($error))
		{
				$q = "SELECT sr_no FROM admin_cred WHERE admin_name = '$n' AND sr_no!= $id";
			
				$result= @mysqli_query($con,$q);//run the querry
			
		if (mysqli_num_rows($result)==0)
		{	
				$q = "UPDATE admin_cred SET admin_name = '$n', admin_pass = '$p',admin_phone='$ph',
				admin_email='$e' WHERE sr_no='$id' LIMIT 1";
		...
		$q = "SELECT admin_name,admin_pass,admin_phone,admin_email 
		FROM admin_cred WHERE sr_no = $id";
		...
		if(mysqli_num_rows ($result)==1)
		{
			...
			<input type="text" id="admin_name" name="admin_name" size="30" maxlength="50" value="'.$row[0].'"></p>

			<p><br><label class="label" for="admin_pass">Admin Password* : </label>
			<input type="text" id="admin_pass" name="admin_pass" size="20" maxlength="50" value="'.$row[1].'"></p>

			<p><br><label class="label" for="admin_phone">Phone No.* : </label>
			<input type="tel" pattern="[0-9]{3}-[0-9]{7}" id="admin_phone" name="admin_phone" size="15" maxlength="20" value="'.$row[2].'"></p>

			<p><br><label class="label" for="admin_email">Admin Email* : </label>
			<input type="text" pattern="[a-z0-9._%+-]+@[a-z-9.-]+\.[a-z]{2,}$" id="admin_email" name="admin_email" size="30" maxlength="50" required value="'.$row[3].'"></p>
